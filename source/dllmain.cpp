...
bool runsWINE() {
    const char* (*pWineGetVersion)(void);

    HMODULE hntdll = GetModuleHandle("ntdll.dll");
    if (!hntdll)
        return false;

    pWineGetVersion = (const char*(*)())GetProcAddress(hntdll, "wine_get_version");
    if (pWineGetVersion) {
        //const char* version = pWineGetVersion();
        //MessageBoxA(NULL, version, "WINE version", MB_OK);
        return true;
    } else {
        //MessageBoxA(NULL, "no WINE", "title", MB_OK);
        return false;
    }
}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
    ...
    case DLL_PROCESS_ATTACH: {
        ...
        *strrchr(iniPath, '\\') = '\0';
        ...
        //MessageBoxA(NULL, iniPath, "title", MB_OK);
        //MessageBoxA(NULL, dllPath, "title", MB_OK);
        ...
        tData->fZoomStep = static_cast<float>(GetPrivateProfileIntA("Patches", "scrollSpeed", 50, iniPath));
        fFPSLimit = static_cast<float>(GetPrivateProfileIntA("DX", "FPSLimit", 0, iniPath));
        bForceWindowedMode = false; // does not work for Silent Heroes
